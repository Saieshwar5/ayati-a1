...
const correlatedEvents = await readCorrelatedProcessEvents(inputPath, primaryEvents);
...
async function readCorrelatedProcessEvents(path, primary) {
...
function renderReport(path, input) {
  ...
  return [
    "# Context Engine live-test report",
    "",
    `Input: ${path}`,
    `Correlated process events: ${correlatedEvents.length}`,
    "",
    "## Health",
    "",
    findings.length === 0
...
function validate(events, lifecycles) {
  ...
  for (const event of events) {
    if (["run_step_persistence_failed", "startup_recovery_failed"].includes(event.event)) {
  ...
  pair(
    ...
    "run_step_persisted",
    (event) => `${event.runId}:${event.data?.step}`,
    findings,
    new Set(["run_step_persistence_acknowledged"]),
  );
