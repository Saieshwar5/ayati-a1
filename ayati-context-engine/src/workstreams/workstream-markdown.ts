import type { ContextEngineErrorCode } from "../errors.js";
...
export function parseContractMarkdown(input: {
  content: string;
  errorCode: ContextEngineErrorCode;
  document: string;
  maxBytes: number;
  ...
  if (Buffer.byteLength(input.content, "utf8") > input.maxBytes) {
    invalid(input, input.document + " exceeds its size limit.");
  }
...
function parseFrontmatter(
  ...
  input: { errorCode: ContextEngineErrorCode; document: string },
...
function invalid(
  input: { errorCode: ContextEngineErrorCode },
  message: string,
