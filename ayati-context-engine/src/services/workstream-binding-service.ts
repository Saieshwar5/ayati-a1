...
export class WorkstreamBindingService {
  constructor(
    ...
    private readonly observer: ContextEngineObserver = new ContextEngineObserver("context-engine"),
  ) {}
  ...
  private assertBindableRun(
    input: Pick<CreateWorkstreamForRunRequest, "requestId" | "runId">,
  ): void {
...
function invalid(
  message: string,
  input: { runId: string; workstreamId?: string },
