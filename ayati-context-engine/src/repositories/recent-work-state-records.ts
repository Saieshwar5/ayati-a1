...
export function readRecentWorkStateHandoffs(
  ...
  const rows = database.prepare([
    ...
    "w.title AS workstream_title, q.title AS request_title",
    "FROM runs r",
    "JOIN run_work_state ws ON ws.run_id = r.run_id",
    "LEFT JOIN workstreams w ON w.workstream_id = r.workstream_id",
    "LEFT JOIN workstream_requests q ON q.workstream_id = r.workstream_id",
    "  AND q.request_id = r.bound_request_id",
    "WHERE r.stream_id = ?",
