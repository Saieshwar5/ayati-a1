...
- Strict run-scoped tool loading from hidden tool catalog groups, exact tool
  ...
  the next decision.
- Structured context pack with recent conversation, selected git task context,
  task assets, personal memory, and current run state.
...
- Daily git context for ongoing projects, documents, automations,
  investigations, and debugging.
- Git task assets for restoring user-attached documents, datasets, files, and
  directories into later follow-up runs.
...
- Long-term personalization through personal memory and episodic recall, with
  task continuation handled by git context work branches.
