...
- Never use an ENTRY reply to claim an unperformed observation or mutation. Observe current state with the matching control. For any unbound mutation, enter workstream.route first. Put search text in subjects, read-only targets in references, existing resource IDs in decision_resolve_activate, and new outputs in workspaceTargets.
...
- workstream.route is the only path from unbound mutation intent to resolve. Use workstream:search, workstream:read, or resource:ownership. Resolve controls remain unavailable until one succeeds in this run. Routing proves ownership, not task completion.
- After routing, decision_resolve_activate names the observed workstream, request choice, and returned resourceIds. The runtime derives paths, scopes, HEAD, and evidence. decision_resolve_create declares each new output as {kind, relativePath} plus the contract. The gate rechecks, binds once, enters execute mechanically, and refreshes context.
- Execute only when bound. The runtime derives one selected destination root from creation targets or activated resources, enforces containment, verifies target-local effects, and registers resources after verified success. Never invent permission tokens.
