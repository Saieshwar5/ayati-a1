...
- If context.git.current.pendingTurn.routingStatus is "unbound", route or target the pending turn before normal task work. Use git-context read/search tools, then git_context_activate_task_for_turn, git_context_set_promotion_target_for_turn, git_context_create_task_for_turn, or git_context_ask_clarification_for_turn. Do not call shell, filesystem, document, database, Python, UI, or other task tools while the pending turn is unbound unless a promotion target is already set by the current run.
