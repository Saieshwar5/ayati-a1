...
export type TurnRoutingUpdate =
  | {
      ...
      runId: string;
      workstreamHead?: string;
      workstreamCreated?: boolean;
      requestDecision?: "initial" | "continue" | "create";
      requestId?: string;
      requestStatus?: "queued" | "active" | "blocked" | "done" | "dropped";
...
export function extractTurnRoutingUpdate(calls: ActToolCallRecord[]): TurnRoutingUpdate | null {
  for (const call of [...calls].reverse()) {
    ...
    if (record["status"] === "ready") {
      ...
      const workstreamId = readString(record["workstreamId"]);
      const contextRepositoryPath = readString(record["contextRepositoryPath"]);
      const branch = readString(record["branch"]);
      const runId = readString(record["runId"]);
      const mode = readSelectionMode(record["mode"]);
      const workstreamHead = readString(record["workstreamHead"]);
      const requestDecision = readRequestDecision(record["requestDecision"]);
      const requestId = readString(record["requestId"]);
      const requestStatus = readWorkstreamRequestStatus(record["requestStatus"]);
      if (sessionId && workstreamId && contextRepositoryPath && branch && runId) {
        return {
          status: "ready",
          sessionId,
          workstreamId,
          ...
          ...(typeof record["workstreamCreated"] === "boolean"
            ? { workstreamCreated: record["workstreamCreated"] }
            : {}),
          ...(requestDecision ? { requestDecision } : {}),
          ...(requestId ? { requestId } : {}),
          ...(requestStatus ? { requestStatus } : {}),
          ...(typeof record["requestCreated"] === "boolean"
            ? { requestCreated: record["requestCreated"] }
            : {}),
          ...(Array.isArray(record["resources"]) ? { resources: record["resources"] } : {}),
          harnessContext,
        };
      }
    }
...
function readWorkstreamRequestStatus(value: unknown): "queued" | "active" | "blocked" | "done" | "dropped" | undefined {
...
function validatePendingRoutingAction(input: ExecutePendingRoutingActionInput): string | undefined {
  ...
  const routingAttemptBlock = validateRoutingAttemptLimits(input.state, action, isWorkstreamBound(input.state));
  ...
  for (const call of action.calls) {
    ...
    if (input.observationalSessionAction) {
      ...
      return [
        `Tool '${call.tool}' cannot run while workstream ownership is unbound or clarifying.`,
        "Inspect workstream/resource candidates, then activate or create a workstream. Ask the user if ownership is unclear.",
      ].join(" ");
    }
