...
export const REPAIR_CODE_CATALOG: Readonly<Record<RepairCode, RepairCatalogEntry>> = {
  ...
  R_EDIT_TARGET_RECOVERY: {
    ...
    allowedNextActions: [
      ...
      "Use write_files with complete desired content when a full-file replacement is clearer than another precise patch attempt.",
    ],
    modelFacing: true,
  },
  R_EDIT_ESCALATE_TO_GUARDED_REWRITE: {
    ...
    allowedNextActions: [
      ...
      "Call read_files with one file in mode=\"full\" for the failed file to get its complete current content.",
