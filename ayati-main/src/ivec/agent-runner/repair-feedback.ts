...
const UNBOUND_RUN_TOOL_REPAIR_MESSAGE = "The run is not bound to a workstream. Observe ownership with workstream:search, workstream:read, or resource:ownership. After successful current-run evidence, enter control-only workstream.route and use the exact binding-required capability with a typed activation or creation proposal. Existing activation names exact routed resource IDs; the runtime derives authority and evidence. Creation needs typed workspace targets. Validate the deterministic gate's focused clarification if ownership remains unclear.";
