...
export function validateRoutingAttemptLimits(
  state: LoopState,
  action: AgentAction,
  hasWorkstreamBinding: boolean,
): RoutingAttemptBlock | undefined {
  const tools = routingControlToolsForAction(action);
  if (tools.length === 0) {
    return undefined;
  }
  if (hasWorkstreamBinding) {
    return {
      reason: "workstream_binding_already_exists",
      message: "Workstream routing tools cannot run after this run is already bound to a workstream.",
      tools,
    };
  }
  if (tools.length > 1) {
    return {
      ...
      message: "Use exactly one workstream routing tool per routing decision.",
      tools,
    };
  }
  if (state.routingAttempts.resolved || state.routingAttempts.successCount > 0) {
    return {
      ...
      message: "Workstream routing is already resolved for this turn; do not call create or activate again.",
      tools,
    };
  }
  if (state.routingAttempts.failureCount >= state.routingAttempts.maxFailures) {
    return {
      ...
      message: `Workstream routing retry limit reached for this turn after ${state.routingAttempts.failureCount} failed attempt(s). Ask the user a short clarification or explain the blocker instead of calling create or activate again.`,
      tools,
    };
  }
  return undefined;
