...
const DEFAULT_SKILL_CARDS: Record<string, Partial<SkillCard>> = {
  ...
  recall: {
    ...
    whenToUse: "Use to search prior conversation/work memory or inspect episodic memory status.",
  ...
  memory: {
    ...
    notFor: ["Do not use for temporary run state; use WorkState instead."],
    risk: "workspace_mutating",
    defaultScope: "run",
  },
  python: {
    ...
    notFor: ["Do not use for simple project commands or direct file edits."],
    risk: "workspace_mutating",
    defaultScope: "run",
  },
  process: {
    title: "Project Processes",
    summary: "Run focused project executables and manage long-running process lifecycles.",
    domains: ["process"],
    triggers: ["run", "build", "test", "lint", "install", "start server", "process output"],
    whenToUse: "Use for project execution only when no filesystem, database, Python, Context Engine, or other focused tool owns the operation.",
    notFor: ["Do not use for file reads/search/mutation, raw Git, SQLite, Python, URL fetches, or shell command strings."],
    risk: "workspace_mutating",
    defaultScope: "run",
  },
  attachments: {
    ...
    whenToUse: "Use when the current work continues from user-provided or previously used attachments.",
  ...
  "git-context": {
    ...
    summary: "Inspect Ayati agent-stream history, durable workstreams, linked resources, requests, and exact evidence.",
    domains: ["git_context"],
    triggers: ["context engine", "git context", "workstream", "workstream request", "resource ownership", "agent history", "durable work"],
    whenToUse: "Use when the agent needs older stream discussion/evidence, workstream candidates, resource ownership, or request history.",
    notFor: ["Do not use read-only inspection tools for workstream binding, context commits, finalization, or editing user files."],
    risk: "read_only",
    defaultScope: "run",
  },
  files: {
...
export class SkillCatalog {
  ...
  promptBlock(): string {
    ...
    return [
      ...
      "Use skill_activate when a card clearly matches the current work and full tool schemas are needed.",
