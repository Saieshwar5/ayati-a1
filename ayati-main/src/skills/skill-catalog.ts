...
const DEFAULT_SKILL_CARDS: Record<string, Partial<SkillCard>> = {
  ...
  process: {
    ...
    whenToUse: "Use for project execution only when no filesystem, database, Python, Git Context, or other focused tool owns the operation.",
  ...
  "git-context": {
    title: "Git Context",
    summary: "Inspect Ayati session context, durable workstreams, linked resources, requests, and recent evidence.",
    domains: ["git_context"],
    triggers: ["git context", "workstream", "workstream request", "resource ownership", "session context", "durable work"],
    whenToUse: "Use when the agent needs to inspect durable session context, workstream candidates, resource ownership, or request history.",
