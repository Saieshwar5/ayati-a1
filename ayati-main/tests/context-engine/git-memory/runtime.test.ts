...
describe("GitMemoryRuntime", () => {
  ...
  it("starts a fresh pending turn for answers to task clarification", async () => {
    const contextStoreDir = await mkdtemp(join(tmpdir(), "ayati-git-memory-runtime-"));
    const runtime = createGitMemoryRuntime({
      contextStoreDir,
      timezone: "Asia/Kolkata",
    ...
    const apiRoute = await runtime.routeUserTurn({
      sessionId: api.sessionId,
      userMessage: "Fix upload API",
      fromSeq: api.userMessage.seq,
      toSeq: api.userMessage.seq,
      at: "2026-06-28T09:00:01+05:30",
    });
    if (apiRoute.status !== "ready") {
      throw new Error(`Expected ready task route, got ${apiRoute.status}.`);
    }
    await runtime.finalizeTaskRun({
      sessionId: api.sessionId,
      ...
      at: "2026-06-28T09:01:00+05:30",
      assistantMessage: "Upload API task noted.",
      assistantAt: "2026-06-28T09:01:05+05:30",
    });
    const ui = await runtime.prepareUserTurn({
      userMessage: "Fix upload UI",
      at: "2026-06-28T09:05:00+05:30",
    });
    const uiRoute = await runtime.routeUserTurn({
      sessionId: ui.sessionId,
      userMessage: "Fix upload UI",
      fromSeq: ui.userMessage.seq,
      toSeq: ui.userMessage.seq,
      at: "2026-06-28T09:05:01+05:30",
    });
    if (uiRoute.status !== "ready") {
      throw new Error(`Expected ready task route, got ${uiRoute.status}.`);
    }
    await runtime.finalizeTaskRun({
      sessionId: ui.sessionId,
      ...
      at: "2026-06-28T09:06:00+05:30",
      assistantMessage: "Upload UI task noted.",
      assistantAt: "2026-06-28T09:06:05+05:30",
    });
    const ambiguous = await runtime.prepareUserTurn({
      userMessage: "continue upload",
      at: "2026-06-28T09:10:00+05:30",
    });
    const clarification = await runtime.askClarificationForTurn({
      sessionId: ambiguous.sessionId,
      reason: "The upload follow-up could refer to the API or UI task.",
      candidateTaskIds: [apiRoute.taskId, uiRoute.taskId],
      at: "2026-06-28T09:10:01+05:30",
    });
    expect(clarification.memoryState.pendingTurn).toMatchObject({
      fromSeq: ambiguous.userMessage.seq,
      toSeq: ambiguous.userMessage.seq,
    ...
    const resolved = await runtime.activateTaskForTurn({
    ...
    expect(resolved).toMatchObject({
      status: "ready",
      taskId: apiRoute.taskId,
      conversationRefs: [{ fromSeq: answer.userMessage.seq, toSeq: answer.userMessage.seq }],
      memoryState: {
        pendingTurn: {
          fromSeq: answer.userMessage.seq,
          toSeq: answer.userMessage.seq,
          text: "the API one",
          routingStatus: "bound",
          taskId: apiRoute.taskId,
        },
    ...
    expect(resolved.conversationRefs).not.toEqual([
      { fromSeq: ambiguous.userMessage.seq, toSeq: ambiguous.userMessage.seq },
    ]);
  });
});
