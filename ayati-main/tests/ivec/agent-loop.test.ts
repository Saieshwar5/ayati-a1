...
describe("agentLoop", () => {
  ...
  it("feeds recent output context cards and evidence refs into the next decision", async () => {
    ...
    try {
      ...
      expect(stateView.context.run.toolCalls.latest).toHaveLength(2);
      expect(stateView.context.run.toolCalls.latest[0].tool).toBe("read_file");
      expect(stateView.context.run.toolCalls.latest[0].input).toEqual({ path: "/proc/meminfo" });
      expect(stateView.context.run.toolCalls.latest[0].output).toContain("3.5Gi used");
      expect(stateView.context.run.toolCalls.latest[0].evidenceRef.ref).toBe("evidence://ev_001_call_1");
      expect(stateView.context.run.toolCalls.latest[1].tool).toBe("search_in_files");
      expect(stateView.context.run.toolCalls.latest[1].input).toEqual({ path: "/proc", query: "memory" });
      expect(stateView.context.run.toolCalls.latest[1].output).toContain("chromium");
      expect(stateView.context.run.toolCalls.latest[1].evidenceRef.ref).toBe("evidence://ev_001_call_2");
