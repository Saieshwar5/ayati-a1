...
describe("stable decision system prompt", () => {
  it("keeps the complete current task protocol without legacy context or controls", () => {
    const prompt = STABLE_DECISION_SYSTEM_CONTEXT;

    expect(prompt).toContain("Task ownership and request lifecycle:");
    ...
    expect(prompt).toContain("git_context_activate_task");
    expect(prompt).toContain("git_context_create_task");
  ...
  it("separates absolute execution paths from portable task assets", () => {
    const prompt = STABLE_DECISION_SYSTEM_CONTEXT;

    expect(prompt).toContain("filesystem and command tool paths are canonical absolute host paths");
    expect(prompt).toContain("durable task assets are different");
    expect(prompt).toContain('"path": "index.html"');
    expect(prompt).not.toContain('"path": "/absolute/task/path/index.html"');
  });
