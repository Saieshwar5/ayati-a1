...
export class WorkstreamFinalizationService {
  ...
  async finalize(
    ...
    const pending = beginRecoverableIdempotent<FinalizeRunResponse | { runId: string }>({
      ...
      execute: () => {
        ...
        updateConversationContentHash(this.options.database, conversation.conversationId, conversationHash);
  ...
  private normalize(input: FinalizeRunRequest): BoundFinalizeInput {
    ...
    if (!run || run.sessionId !== input.sessionId || !binding || !completion) {
  ...
  private async prepare(input: BoundFinalizeInput, _session: SessionRef) {
    const run = readRunEvidence(this.options.database, input.runId);
...
function closeConversation(
  ...
  return input.assistantResponse
    ? closeRunConversationWithAssistant(database, { ...common, content: input.assistantResponse })
...
function invalid(message: string, details?: Record<string, unknown>): GitContextServiceError {
  return new GitContextServiceError({ code: "INVALID_REQUEST", message, ...(details ? { details } : {}) });
}

function recovery(message: string, details?: Record<string, unknown>): GitContextServiceError {
  return new GitContextServiceError({ code: "RECOVERY_REQUIRED", message, ...(details ? { details } : {}) });
}

function headMismatch(workstreamId: string, expected: string, actual: string): GitContextServiceError {
