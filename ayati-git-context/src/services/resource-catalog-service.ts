...
import {
  ...
  readSessionResourcesProjection,
...
export class ResourceCatalogService {
  ...
  async inspect(input: InspectResourceForRunRequest): Promise<InspectResourceForRunResponse> {
    ...
    this.requireActiveRun(input.sessionId, input.runId);
  ...
  bind(input: BindResourcesForRunRequest): BindResourcesForRunResponse {
    return executeIdempotent({
      ...
      now: input.at,
      execute: () => {
        const run = this.requireBoundRun(input.sessionId, input.runId, input.workstreamId);
        return {
  ...
  async ensureManagedOutput(input: {
    ...
    this.requireBoundRun(input.sessionId, input.runId, input.workstreamId);
  ...
  sessionProjection(sessionId: string): SessionResourcesProjection {
    return readSessionResourcesProjection(this.database, sessionId);
  }
  ...
  private requireActiveRun(sessionId: string, runId: string): void {
  ...
  private requireBoundRun(
    ...
    runId: string,
    workstreamId: string,
  ): NonNullable<ReturnType<typeof readRun>> {
    const run = readRun(this.database, runId);
    if (!run || run.sessionId !== sessionId
      ...
      throw new GitContextServiceError({
        ...
        details: { sessionId, runId, workstreamId },
      });
    }
    return run;
  }
