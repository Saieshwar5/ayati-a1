...
export async function rebuildWorkstreamCatalog(input: {
  ...
  if (!input.database) throw new Error("Catalog rebuild confirmation requires the V5 database.");
...
function applyCatalog(
  ...
  if (!session) throw new Error("Catalog rebuild requires an initialized daily session.");
  database.transaction(() => {
    for (const repository of repositories) insertWorkstream(database, repository, session.session_id, now);
...
function insertWorkstream(
  ...
  database.prepare([
    ...
    "current_request_title, current_request_status, status, created_session_id, created_at, updated_at)",
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
  ].join(" ")).run(
