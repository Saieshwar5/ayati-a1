...
export function bindResourcesToWorkstream(
  ...
  if (!run || run.status !== "running" || run.session_id !== input.sessionId
    || run.workstream_id !== input.workstreamId || run.bound_request_id !== input.requestId) {
    throw new GitContextServiceError({
...
export function readSessionResourcesProjection(
  database: ContextDatabase,
  sessionId: string,
  limit = 12,
): SessionResourcesProjection {
  const rows = database.prepare([
    ...
    "JOIN messages m ON m.message_id = mr.message_id WHERE m.session_id = ? GROUP BY mr.resource_id",
    ") recent ON recent.resource_id = r.resource_id",
    "ORDER BY recent.used_at DESC, r.resource_id LIMIT ?",
  ].join(" ")).all(sessionId, limit) as unknown as ResourceRow[];
  const count = database.prepare([
    "SELECT COUNT(DISTINCT mr.resource_id) AS count FROM message_resources mr",
    "JOIN messages m ON m.message_id = mr.message_id WHERE m.session_id = ?",
