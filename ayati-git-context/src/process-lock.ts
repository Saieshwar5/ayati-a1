...
export class GitContextProcessLock {
  ...
  static async acquire(input: {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        ...
        return new GitContextProcessLock(input.path, metadata, recoveredStaleOwner);
      } catch (error) {
        if (!isAlreadyExists(error)) throw error;
        const owner = await readLockMetadata(input.path);
        if (owner && isProcessAlive(owner.pid)) {
          throw new Error(
            "Git Context Engine storage already has a live writer (pid "
    ...
    throw new Error("Could not acquire Git Context Engine writer lock: " + input.path);
  }
